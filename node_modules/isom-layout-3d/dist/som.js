(function (global, factory) {
	...
	(global.som3D = factory());
}(this, (function () { 'use strict';

function randomize3D (nodes, ref) {
  ...
  for (let i = 0, len = nodes.length; i < len; i++) {
    ...
    node.z = minZ + random() * dz;
  }
}


function som3D (data, ref) {
  ...
  function update (data, map, bounds) {
    ...
    var tmp = {
      x: bounds[0] + Math.random() * (bounds[3] - bounds[0]),
      y: bounds[1] + Math.random() * (bounds[4] - bounds[1]),
      z: bounds[2] + Math.random() * (bounds[5] - bounds[2]),
    };
    ...
    for (var i = 0, len = nodes.length; i < len; i++) {
      ...
      var dy = tmp.y - node.y;
      var dz = tmp.z - node.z;
      var localSqDist = dx * dx + dy * dy + dz * dz;
      if (localSqDist < dist) {
        dist   = localSqDist;
        winner = node;
      }
  ...
  if (!dontRandomize) { randomize3D(data, bounds); }
